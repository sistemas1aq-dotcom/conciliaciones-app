...
import { Sede } from '../../../models/activo-fijo.model';
    ...
    <div class="space-y-4">
      ...
      <div class="flex items-center justify-between">
        <div>
          ...
          <p class="text-sm text-gray-500">Gestionar sedes de la empresa</p>
        </div>
      ...
      <div class="flex items-center gap-3">
        <div class="relative flex-1 max-w-sm">
          ...
          <input [(ngModel)]="searchTerm" (ngModelChange)="applyFilter()" type="text" placeholder="Buscar sede..." class="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none">
        </div>
          ...
          <table class="w-full text-sm">
            <thead>
              <tr class="bg-[#1a3a5f] text-white">
                ...
                <th class="text-left px-4 py-3 font-semibold text-xs uppercase tracking-wider">Direccion</th>
            ...
            <tbody>
              @for (item of paginatedData; track item.id) {
                <tr class="border-b border-gray-100 hover:bg-gray-50 transition">
                  ...
                  <td class="px-4 py-3 text-gray-600">{{ item.direccion }}</td>
        ...
        <div class="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" (click)="closeModal()">
          <div class="bg-white rounded-xl shadow-2xl w-full max-w-lg" (click)="$event.stopPropagation()">
            <div class="flex items-center justify-between px-6 py-4 border-b border-gray-200">
              <h2 class="text-lg font-semibold text-gray-900">{{ editing ? 'Editar Sede' : 'Nueva Sede' }}</h2>
            ...
            <div class="p-6 space-y-4">
              ...
              <div>
                ...
                <input [(ngModel)]="form.direccion" type="text" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none">
              </div>
              <div class="flex items-center gap-2">
...
export class SedesComponent implements OnInit {
  ...
  form: Sede = { id: 0, codigo: '', descripcion: '', direccion: '', activo: true };
  ...
  applyFilter() {
    ...
    this.filteredData = t
      ? this.data.filter(s => s.codigo.toLowerCase().includes(t) || s.descripcion.toLowerCase().includes(t) || s.direccion.toLowerCase().includes(t))
      : [...this.data];
    this.currentPage = 1;
  ...
  openModal() {
    this.editing = false;
    this.form = { id: 0, codigo: '', descripcion: '', direccion: '', activo: true };
    this.showModal = true;
  }
  ...
  save() {
    ...
    if (this.editing) {
      this.svc.updateSede(this.form);
    } else {
      this.svc.addSede(this.form);
    }
    this.closeModal();
    this.loadData();
  }
