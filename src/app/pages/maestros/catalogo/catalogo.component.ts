...
import { CatalogoItem } from '../../../models/activo-fijo.model';
  ...
  template: `
    <div class="space-y-4">
      <div class="flex items-center justify-between">
        <div>
          <h1 class="text-2xl font-bold text-gray-900">Catalogo</h1>
          <p class="text-sm text-gray-500">Gestionar catalogo de activos fijos</p>
        </div>
      ...
      <div class="flex items-center gap-3">
        <div class="relative flex-1 max-w-sm">
          ...
          <input [(ngModel)]="searchTerm" (ngModelChange)="applyFilter()" type="text" placeholder="Buscar en catalogo..." class="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none">
        </div>
          ...
          <table class="w-full text-sm">
            <thead>
              <tr class="bg-[#1a3a5f] text-white">
                ...
                <th class="text-left px-4 py-3 font-semibold text-xs uppercase tracking-wider">Cuenta</th>
            ...
            <tbody>
              @for (item of paginatedData; track item.id) {
                <tr class="border-b border-gray-100 hover:bg-gray-50 transition">
                  ...
                  <td class="px-4 py-3 font-mono text-xs text-gray-600">{{ item.cuenta }}</td>
        ...
        <div class="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" (click)="closeModal()">
          <div class="bg-white rounded-xl shadow-2xl w-full max-w-lg" (click)="$event.stopPropagation()">
            <div class="flex items-center justify-between px-6 py-4 border-b border-gray-200">
              <h2 class="text-lg font-semibold text-gray-900">{{ editing ? 'Editar Catalogo' : 'Nuevo Catalogo' }}</h2>
            ...
            <div class="p-6 space-y-4">
              ...
              <div>
                ...
                <input [(ngModel)]="form.cuenta" type="text" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none">
              </div>
              <div class="flex items-center gap-2">
...
export class CatalogoComponent implements OnInit {
  protected Math = Math;
  data: CatalogoItem[] = [];
  filteredData: CatalogoItem[] = [];
  paginatedData: CatalogoItem[] = [];
  searchTerm = '';
  showModal = false;
  editing = false;
  currentPage = 1;
  ...
  form: CatalogoItem = { id: 0, codigo: '', descripcion: '', cuenta: '', activo: true };
  ...
  loadData() {
    this.data = this.svc.getCatalogo();
    this.applyFilter();
  }
  ...
  applyFilter() {
    ...
    this.filteredData = t
      ? this.data.filter(c => c.codigo.toLowerCase().includes(t) || c.descripcion.toLowerCase().includes(t) || c.cuenta.toLowerCase().includes(t))
      : [...this.data];
    this.currentPage = 1;
  ...
  openModal() {
    this.editing = false;
    this.form = { id: 0, codigo: '', descripcion: '', cuenta: '', activo: true };
    this.showModal = true;
  }
  ...
  save() {
    ...
    if (this.editing) {
      this.svc.updateCatalogo(this.form);
    } else {
      this.svc.addCatalogo(this.form);
    }
    this.closeModal();
    this.loadData();
  }
