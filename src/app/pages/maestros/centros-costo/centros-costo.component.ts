...
import { CentroCosto } from '../../../models/activo-fijo.model';
    ...
    <div class="space-y-4">
      <div class="flex items-center justify-between">
        <div>
          ...
          <p class="text-sm text-gray-500">Gestionar centros de costo</p>
        </div>
      ...
      <div class="flex items-center gap-3">
        <div class="relative flex-1 max-w-sm">
          ...
          <input [(ngModel)]="searchTerm" (ngModelChange)="applyFilter()" type="text" placeholder="Buscar centro de costo..." class="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none">
        </div>
          ...
          <table class="w-full text-sm">
            ...
            <tbody>
              @for (item of paginatedData; track item.id) {
                ...
                <tr><td colspan="4" class="px-4 py-8 text-center text-gray-400">No se encontraron registros</td></tr>
              }
            </tbody>
          </table>
        </div>
        ...
        <div class="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" (click)="closeModal()">
          <div class="bg-white rounded-xl shadow-2xl w-full max-w-lg" (click)="$event.stopPropagation()">
            <div class="flex items-center justify-between px-6 py-4 border-b border-gray-200">
              <h2 class="text-lg font-semibold text-gray-900">{{ editing ? 'Editar Centro de Costo' : 'Nuevo Centro de Costo' }}</h2>
...
export class CentrosCostoComponent implements OnInit {
  protected Math = Math;
  data: CentroCosto[] = [];
  filteredData: CentroCosto[] = [];
  paginatedData: CentroCosto[] = [];
  searchTerm = '';
  showModal = false;
  editing = false;
  currentPage = 1;
  ...
  form: CentroCosto = { id: 0, codigo: '', descripcion: '', activo: true };
  ...
  loadData() {
    this.data = this.svc.getCentrosCosto();
    this.applyFilter();
  }
  ...
  applyFilter() {
    ...
    this.filteredData = t
      ? this.data.filter(c => c.codigo.toLowerCase().includes(t) || c.descripcion.toLowerCase().includes(t))
      : [...this.data];
    this.currentPage = 1;
  ...
  openModal() {
    this.editing = false;
    this.form = { id: 0, codigo: '', descripcion: '', activo: true };
    this.showModal = true;
  }

  editItem(item: CentroCosto) {
    this.editing = true;
    this.form = { ...item };
    this.showModal = true;
  }
  ...
  save() {
    ...
    if (this.editing) {
      this.svc.updateCentroCosto(this.form);
    } else {
      this.svc.addCentroCosto(this.form);
    }
    this.closeModal();
    this.loadData();
  }
